      ...
      <div className="flex items-end justify-between">
        <div>
          <p className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">
            Pipeline overview · {stats.days_back}d data window
          </p>
        ...
        <div className="flex items-center gap-2 text-[11.5px] text-muted-foreground">
          <span
            className={`size-1.5 rounded-full ${isLoading ? "bg-muted-foreground/40" : "bg-[oklch(0.62_0.13_155)]"}`}
          />
          {isLoading ? "Loading…" : `Pipeline refreshed ${formatDateTime(generatedAt)}`}
        </div>
      </div>

      {/* Metric strip */}
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-px bg-border border border-border rounded-md overflow-hidden">
        <MetricCell
          label="Ingested"
          value={stats.ingested.toLocaleString()}
          sub={`${stats.days_back}d window`}
        />
        <MetricCell
          label="Filtered"
          value={stats.filtered.toLocaleString()}
          sub={`${Math.round(passThroughRate * 100)}% pass-through`}
        />
        <MetricCell
          label="Qualified"
          value={stats.qualified.toString()}
          sub={`${stats.surfaced} surfaced`}
          trend="up"
        />
        <MetricCell
          label="Hot opps"
          value={tiers.hot.toString()}
          sub="Status tier"
        ...
        <MetricCell
          label="Warm opps"
          value={tiers.warm.toString()}
          sub="Status tier"
        />
        <MetricCell
          label="Active opps"
          value={stats.surfaced.toString()}
          sub="Latest run"
          icon={Activity}
        />
      </div>
            ...
            >
              ...
              <Bar dataKey="count" radius={[2, 2, 0, 0]}>
                {statusDistribution.map((d, i) => {
                  const colors = ["#339CEC", "#232323", "#de422f", "#8a8a8a", "#bbb"];
                  return <Cell key={d.status} fill={colors[i % colors.length]} />;
                })}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <Panel title="Score distribution" subtitle="Match score bands">
          <div className="space-y-3 pt-2">
            {scoreDistribution.map((b, i) => {
              const max = Math.max(...scoreDistribution.map((x) => x.count));
              const w = (b.count / max) * 100;
              const color = i === 0 ? "#de422f" : i === 1 ? "#339CEC" : "#232323";
              return (
                <div key={b.bucket}>
                  <div className="flex items-baseline justify-between text-[11.5px] mb-1">
                    ...
                    <span className="num font-medium text-charcoal">{b.count}</span>
                  </div>
          ...
          <tbody className="divide-y divide-border">
            {topLeads.map((o) => (
              <tr key={o.id} className="hover:bg-secondary/40">
                <td className="py-2.5 pl-1 num text-muted-foreground">{o.rank}</td>
                ...
                <td className="py-2.5 pl-3">
                  <StatusTierBadge tier={o.status_tier} />
                </td>
              </tr>
            ))}
          </tbody>
...
function Panel({
  ...
  return (
    <section className={`bg-card border border-border rounded-md p-4 ${className}`}>
  ...
  return (
    <div className="bg-card p-3.5">
      <div className="flex items-center justify-between">
        ...
        {Icon && (
          <Icon className="size-3.5 text-muted-foreground/60" strokeWidth={1.75} />
        )}
      </div>
      <div className={`text-[20px] font-semibold tracking-tight num mt-1 ${accent ?? "text-charcoal"}`}>
        {value}
      </div>
      {sub && (
        <div className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-1">
          {trend === "up" && <ArrowUp className="size-3 text-[oklch(0.5_0.13_155)]" />}
          {trend === "down" && <ArrowDown className="size-3 text-[oklch(0.5_0.21_28)]" />}
          {sub}
        </div>
      )}
    </div>
